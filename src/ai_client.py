# -*- coding: utf-8 -*-
r"""ai_client.py — 밖(AI)과 통신하는 유일한 출구 + 마스킹 게이트
...
불변 2의 정신(비밀은 창고·저장소에 안 넣는다): 열쇠는 %USERPROFILE%\.fastpoten\api_key.txt
  에서만 읽고, 로그·예외 메시지·창고 어디에도 적지 않는다(앞 4자·길이도 남기지 않는다).
  ★ 창고(data/)에 두지 않는다 — data/는 구글드라이브로 옮겨지는 폴더라 열쇠가 클라우드에
    실린다(계획 §Q4: 창고·저장소·구글드라이브 어디에도 두지 않는다).
  ★ 환경변수 ANTHROPIC_API_KEY에도 두지 않는다 — 2026-07-21에 클로드 코드가 그 열쇠를
    발견해 자기 대화에 쓰는 바람에 원고 한 편 뽑기 전에 요금이 나갔다. 이 열쇠는
    원고 생성 전용이고, 다른 프로그램 눈에 띄지 않는 자리에 둔다.
...
# 열쇠 자리(계획 §Q4). 저장소·구글드라이브·창고 밖 — PC마다 한 번만 두면 된다.
KEY_PATH = os.path.join(os.path.expanduser("~"), ".fastpoten", "api_key.txt")
...
def read_key(path=None):
    """열쇠를 읽는 유일한 곳. 값은 돌려주기만 하고 안 적는다.

    환경변수는 일부러 보지 않는다 — 거기 두면 다른 프로그램(클로드 코드 등)이
    발견해 제 통신에 써버린다(2026-07-21 사고). 열쇠 자리는 파일 하나뿐이다.

    파일은 사람이 메모장·파워셸로 만든다. 그래서 BOM(utf-8-sig)을 견디고,
    메모 줄이 섞여도 첫 내용 줄만 쓰며, 읽기 실패는 예외 없이 ""로 돌려준다
    (열쇠가 없거나 깨졌을 때 화면이 죽지 않고 no_key 안내로 떨어지게).
    """
    try:
        with open(path or KEY_PATH, encoding="utf-8-sig") as f:
            lines = [ln.strip() for ln in f if ln.strip()]
    except (IOError, OSError, ValueError):        # ValueError=인코딩 깨짐(UTF-16 등)
        return ""
    # 열쇠처럼 생긴 줄이 있으면 그 줄(메모가 섞여 있어도 안전), 없으면 첫 줄
    return next((ln for ln in lines if ln.startswith("sk-")), lines[0] if lines else "")
...
def _make_client():
    ...
    key = read_key()
    if not key:
        return None, "no_key", ("AI 열쇠가 없습니다. %s 파일에 열쇠만 한 줄로 적은 뒤 "
                                "다시 시도하세요." % KEY_PATH)
    try:
        import anthropic
    except ImportError:
...
def ask(conn, prompt, max_tokens=DEFAULT_MAX_TOKENS, client=None,
    ...
    client는 테스트에서 가짜를 넣기 위한 자리(실사용에서는 None → 열쇠 파일로 만든다).
    """
